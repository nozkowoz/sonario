...
- **"You're here" shouldn't outlive the rehearsal.** Nina, 2026-09-15 (found live-testing check-in): after the event's end time (she saw it past 9pm), the check-in status still reads "You're here" — should stop showing that once the rehearsal is over. Small bug, not a new feature — needs the check-in badge logic to compare against the event's end_time, not just whether a checkins row exists.
- **Editable check-in time (best-guess correction).** Nina, 2026-09-15: if someone forgets to check in live, let them enter their best-guess arrival time afterwards. Flagging a real conflict before this gets built: migration 0003's `enforce_checkin_timestamp` trigger deliberately overwrites `checked_in_at` with the server clock for every `source = 'live'` insert specifically so the timestamp (and the punctuality tier computed from it, see `checkinTier` in `js/lib.js`) can't be backdated or spoofed — client-editable time cuts directly against that guarantee. Needs a real decision: a distinct "guessed" source/state that's visually marked as unverified and maybe excluded from punctuality tiering, vs. loosening the trigger for everyone. Don't build either without that decision.
