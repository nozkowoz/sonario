...
- **Voice Memos upload fix (v43) didn't actually work — needs re-doing.** Nina, 2026-09-16, tested live: widening the file input to `accept="audio/*"` did NOT surface Voice Memos as a source on iOS; it just added Photos/Videos as options instead, which she doesn't want. The real fix is still unsolved — needs real on-device iteration (this can't be diagnosed blind from code, iOS's file-picker source list is notoriously undocumented/inconsistent across versions). `js/repertoire.js`'s `AddRecordingSheet` file input, currently `accept="audio/*"`.
