...
- **Workshops and concerts/performances should be blue.** Nina, 2026-09-17: currently `.event-type-workshop` is lilac and `.event-type-performance` is green (`css/styles.css`). Worth noting: this reverses an earlier deliberate choice — Nina's original Figma painted every non-rehearsal event the same sky blue, and the team moved to per-type semantic colours instead when she was asked (see the comment on `rowTintClass` in `js/events.js`). Confirm this is a genuine change of mind before just reverting.
...
- **Voice Memos upload fix (v43) didn't actually work, and briefly regressed Files upload too.** Nina, 2026-09-16, tested live: widening the file input to `accept="audio/*"` did NOT surface Voice Memos as a source on iOS, added unwanted Photos/Videos options, AND broke the previously-working "pick from Files" path (reverted immediately after, in the same commit as this TODO note). Voice Memos as a source is still unsolved — needs real on-device iteration, not blind code changes; iOS's file-picker source list is notoriously undocumented/inconsistent across versions. `js/repertoire.js`'s `AddRecordingSheet` file input is back to its original working `accept` list.
- **Remove the "Tip" callout on the Lyrics tab.** Nina, 2026-09-16: "you can paste lyrics from a document or type them directly here" — take it out. `js/repertoire.js`'s `LyricsTab`, the `.lyrics-tip-card` block.
- **Recordings list still shows "Barry" (e.g. under Love Ruins Everything).** Same root cause as the Practice Mode sighting — migration `0010_bari_label.sql` hasn't been pasted into the live database yet. Not a separate bug.
